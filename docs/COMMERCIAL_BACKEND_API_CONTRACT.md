...
历史版本残留的代理登录态不得写入或继续保留在 `profile.json`。保存 profile 时必须按白名单重建 payload；如果旧 `profile.json` 已混入代理身份、代理 token、买家登录 token、`assist_session_id`、邀请码、订单号、权益 ID 或配置会话 ID，下一次保存必须清除，不能继续带入长期登录态。启动恢复时不得把旧代理登录态或旧买家 token 当成当前账号，只能恢复账号提示、API Key、模型和界面偏好；真正的登录、授权和部署 token 必须来自本次重新登录。
