...
“胖虎AI”是一款面向买家和新手用户的一站式 AI 客户端服务工具。它不是单纯的 Agent 配置器，而是把 AI 使用过程中常见的下载、账号、API、中转、充值、会员、接码、手机控制和交付验收卡点集中到一个登录后的客户端里处理。
...
- 登录后可在“配置Agent -> 手机控制Agent”进入独立增值服务：把已可用的 Agent 接入 QQ、微信、飞书、钉钉、企业微信等手机通讯或办公通道。该入口不能被“本工具本次基础 Agent 配置是否完成”硬锁死；买家电脑已有可用 Agent 时，也可以走已有 Agent 检测、选择和单独验收链路。
- 通过内置网站入口打开胖虎AI网站的注册、邀请码、创建 API Key、充值购买等服务端页面，并在代理中心承接本工具独立代理业务入口。
- 在“增值业务”中承接 GPT 会员代充、账号服务、手机号/短信接码、手机控制 Agent 等服务入口；商品、价格、履约和上架状态以服务端为准。
...
8. 如客户额外购买手机控制Agent，在独立订单、独立配置会话和独立验收记录中完成手机端消息通道闭环；该项失败不回滚基础 Agent 配置交付。
