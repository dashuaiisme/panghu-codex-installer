...
- 买家电脑原本已有可用 Agent、历史订单已交付 Agent，或人工复核确认 Agent 可用时，也可以直接进入手机控制Agent检测、配置和单独验收。
- 它单独收费。
- 它单独验收。
- 它失败不影响基础 Agent 配置交付。
...
```text
手机控制Agent不能只用“当前能不能收到消息”判断是否收费。配置完成并形成入站消息、Agent 调用、出站回复和 source_event_id 等验收证据后，如果客户自行断网、禁用 API Key、取消平台授权、关闭机器人、删除群聊或阻断回调，只能进入暂停、重试或人工复核，不能自动判定配置失败、自动退款或不收费。反过来，如果从未形成这些验收证据，也不能标记手机控制Agent交付完成。
```
...
```text
如果这台电脑已经有可用 Agent，可直接选择已有 Agent 进行检测并配置手机控制Agent，不需要重新购买或重新安装基础 Agent。
```
...
- 在商业合同和验收文档中明确：已有 Agent / 历史交付 / 人工复核可以进入手机控制Agent链路，不得用本次基础配置完成状态硬锁入口。
...
- 平台能力以服务端配置和实际验收为准。
- 手机控制Agent入口支持已有可用 Agent 检测或人工复核，不与本工具本次基础配置会话强绑定。
...
答：不一定。手机控制Agent可以选择已有 Agent 做运行检测并单独配置手机通道；如果检测不清楚，会进入人工复核或提示先修复 Agent，而不是强制重装。
