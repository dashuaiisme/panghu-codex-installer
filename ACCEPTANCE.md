...
## 7. G 级：手机控制Agent增值服务验收

手机控制Agent必须与基础 Agent 配置分开验收：

- 基础 Agent 配置交付通过，不自动代表手机控制Agent通过。
- 手机控制Agent失败，不自动回滚基础 Agent 配置交付。
- 手机控制Agent必须有独立订单、独立配置会话、独立验收记录和独立收费事件。
...
- 未形成入站消息、Agent 调用和出站回复证据时，不得标记手机控制Agent交付完成。
