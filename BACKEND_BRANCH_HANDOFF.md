...
你负责“胖虎AI多 Agent 一键部署工具”的商业逻辑、服务端合同边界、内置网站闭环可行性、目标 Agent 真实交付链路评审和补齐建议。
...
- `pywebview` 不可用时是否明确阻断并标记内置浏览器未完成，而不是声称完全内置或自动打开系统浏览器。
...
Gemini / agy 当前只保留官方入口和待接入状态；未接入胖虎AI API Key 配置、启动检测、最小中文对话和功能验收矩阵前，不计完整配置交付。
...
- 不把部署授权 token 写入长期文件；买家会话恢复只走独立 cookie/WebView profile。
- `profile.json` 不写密码、第三方账号密码、服务端授权 token、订单号、权益 ID 或配置会话 ID；用户显式勾选“记住密码”时，胖虎AI买家密码只能进入独立 `login_accounts.json` 的本机系统加密 blob。
