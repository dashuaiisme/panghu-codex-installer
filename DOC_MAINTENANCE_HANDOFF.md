...
- 连接通讯软件是“配置Agent”模块内的独立增值服务，不是基础 Agent 配置的一部分，也不是代理中心能力。
- 连接通讯软件入口不能被本工具本次基础配置会话硬锁死；已有可用 Agent、历史交付或人工复核可以进入单独配置和验收链路。
- 连接通讯软件必须独立订单、独立配置会话、独立验收、独立收费；已形成验收证据后，客户断网、禁 Key、取消平台授权或阻断回调不得自动免单。
...
- Mac 未公证时的安全提示。
- 未打通项不能说成已完成。
- 连接通讯软件必须说明为独立增值服务，支持已有 Agent 检测进入，不要求客户必须重新做基础 Agent 安装。
- 连接通讯软件不能说成“当前收不到消息就自动失败/免单”；要区分配置验收证据和客户后续断网、禁 Key、取消授权等外部中断。
...
- 连接通讯软件主入口在 `配置Agent -> 连接通讯软件`。
- 增值业务只展示销售卡片或介绍，正式配置入口回到配置Agent模块。
- 代理中心不得作为连接通讯软件入口。
- 基础 Agent 配置和连接通讯软件是两个服务，分别收费、分别验收、分别交付。
- 两者有推荐先后顺序，但不是硬绑定；已有可用 Agent、历史交付或人工复核可进入连接通讯软件链路。
- 连接通讯软件交付不能只以“当前能不能收到消息”为准；验收证据形成后，客户断网、禁 Key、取消平台授权或阻断回调不得自动退款、免单或判定配置失败。
- 平台支持状态、价格、次数、可售状态和验收策略必须由服务端下发，不能客户端硬编码。

补充搜索：

```powershell
rg -n -g "*.md" -g "*.txt" -g "!legacy/**" -g "!docs/superpowers/**" "连接通讯软件|communication_software_link|communication-software-link|source_event_id|断网|禁 Key|取消平台授权|已有可用 Agent|历史交付|人工复核" .
rg -n -g "*.md" -g "*.txt" -g "!legacy/**" -g "!docs/superpowers/**" "连接通讯软件.*基础 Agent 配置的一部分|Agent 安装成功.*连接通讯软件|连接通讯软件.*共用.*验收|当前.*收不到消息.*失败|requires_base_agent_delivery.*true" .
```
